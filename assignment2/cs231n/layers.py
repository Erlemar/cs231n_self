...
def batchnorm_forward(x, gamma, beta, bn_param):
	...
	if mode == 'train':
		...
		out1 = (x - x_mean) / np.sqrt(x_var + eps)
		out = gamma * out1 + beta
		cache = (x_mean, x_var, gamma, beta, out1, eps, x)
...
def batchnorm_backward(dout, cache):
	...
	###########################################################################
	#cache = (x_mean, x_std, gamma, beta, out1)
	(x_mean, x_var, gamma, beta, out1, eps, x) = cache
	l_x = gamma * dout 
...
def max_pool_forward_naive(x, pool_param):
	...
	for n in range(N):
		for c in range(C):
			for w in range(W_):
				for h in range(H_):
					current_array = x[n, c, w * pool_param['stride']:w * pool_param['stride'] + pool_param['stride'], h * pool_param['stride']:h * pool_param['stride'] + pool_param['stride']]
