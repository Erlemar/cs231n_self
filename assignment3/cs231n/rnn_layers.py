...
def rnn_step_backward(dnext_h, cache):
	...
	next_h, Wx, x, Wh, h = cache
	db = np.sum((1 - next_h * next_h) * dnext_h, axis=0)
	dx = np.dot((1 - next_h * next_h) * dnext_h, Wx.T)
	dWx = np.dot(x.T, (1 - next_h * next_h) * dnext_h)
	dprev_h = np.dot((1 - next_h * next_h) * dnext_h, Wh.T)
	dWh = np.dot(h.T, (1 - next_h * next_h) * dnext_h)
...
def rnn_forward(x, h0, Wx, Wh, b):
	...
	h_temp = h0
	for i in range(T):
		#h[i] = np.tanh(np.dot(x[:,0,:], Wx) + np.dot(h0, Wh) + b)
		#print x.shape, i
		next_h, cache_i = rnn_step_forward(x[:,i,:], h_temp, Wx, Wh, b)
...
def rnn_backward(dh, cache):
	...
	for i in reversed(range(T)):
		#print T-1, i, T
		if i == T-1:
			dx[:,i,:], dprev_h, dWx, dWh, db = rnn_step_backward(dh[:,i,:] + dprev_h, cache[i])
...
def temporal_softmax_loss(x, y, mask, verbose=False):
	...
	minibach and padded with NULL tokens. The optional mask argument tells us
